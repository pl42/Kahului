...
import com.pl42.KahuluiApplication;
import com.pl42.kahului.model.DataIdentifier;
import com.pl42.kahului.model.data.MindData;
import com.pl42.kahului.model.data.PredictionEngine;
...
public class Kahului {
  ...
  private String VERSION = KahuluiApplication.getVersion();
  ...
  /**
   * Resets Kahului's memory. This is necessary to do or memory leaks will be possible
   */
  public void reset() {
  ...
  /**
   * Returns the current state of Kahului. This relates to whether the bot is waiting
   * for a buy back or not
   *
   * @return String for the UI stating Kahului's status
   */
    ...
    if (message.length() <= 280) {
      try {
        if (EXECUTE_TWEETS) {
          ...
          logger.trace("Sent tweet to @WestworldKahului");
        } else {
  ...
  private void reportDevMode() {
    logger.trace("Kahului is currently in development mode! Not performing trades or tweets");
  }
}
